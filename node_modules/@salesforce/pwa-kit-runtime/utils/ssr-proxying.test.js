...
describe('rfc1123 tests', () => {
  ...
  testCases.forEach(testCase => test(`Expecting ${testCase.expected}`, () => {
    expect((0, _ssrProxying.rfc1123)(testCase.date)).toEqual(testCase.expected);
  }));
});
describe('parseHost tests', () => {
  ...
  testCases.forEach(testCase => test(`${testCase.name} (${testCase.host})`, () => {
    expect((0, _ssrProxying.parseHost)(testCase.host)).toEqual(testCase.expected);
  }));
});
describe('cookieAsString tests', () => {
  ...
  testCases.forEach(testCase => test(`Expecting ${testCase.expected}`, () => {
    expect((0, _ssrProxying.cookieAsString)(testCase.cookie)).toEqual(testCase.expected);
  }));
});
describe('rewriteProxyResponseHeaders tests', () => {
  ...
  const tooLongUrl = tooLongUrlBase.padEnd(_ssrProxying.MAX_URL_LENGTH_BYTES, 'x');
  const testCases = [{
    ...
    input: {
      'access-control-allow-origin': '*',
      'set-cookie': ['origin_dc=war; expires=Mon, 01-Oct-2018 00:13:20 GMT; path=/; domain=.www.customer.com', 'origin_dc=war; expires=Tue, 02-Oct-2018 00:13:20 GMT; path=/; domain=.someone.com']
    },
    expected: {
      'access-control-allow-origin': '*',
      'set-cookie': ['origin_dc=war; Path=/; Expires=Mon, 01 Oct 2018 00:13:20 GMT; Domain=apphost.mobify.com', 'origin_dc=war; Path=/; Expires=Tue, 02 Oct 2018 00:13:20 GMT; Domain=.someone.com']
    }
  }, {
    ...
    expected: {
      'x-proxy-request-url': tooLongUrl.slice(0, _ssrProxying.MAX_URL_LENGTH_BYTES)
    }
  }];
  testCases.forEach((testCase, testCaseIndex) =>
  // eslint-disable-next-line jest/valid-title
  test(testCase.name || `test ${testCaseIndex}`, () => {
    const updatedHeaders = (0, _ssrProxying.rewriteProxyResponseHeaders)({
  ...
  test('missing headers', () => {
    expect((0, _ssrProxying.rewriteProxyResponseHeaders)({
      appHostname: '',
      proxyPath: '/mobify/proxy/base',
      targetProtocol: 'https',
      targetHost: 'www.customer.com'
  ...
  test('Headers.modified', () => {
    const headers = new _ssrProxying.Headers({
      a: '1'
    }, 'http');
  ...
  test('Headers case handling', () => {
    const headers = new _ssrProxying.Headers({}, 'aws');
  ...
  test('bad Headers format', () => {
    expect(() => new _ssrProxying.Headers({})).toThrow();
  });
});
describe('rewriteProxyRequestHeaders tests', () => {
  ...
  // eslint-disable-next-line jest/valid-title
  test(testCase.name || `test ${testCaseIndex}`, () => {
    const headers = _extends({}, testCase.input || {});

    // If we're testing the allowlist, add a value for
    // every allowed header.
    if (testCase.testAllowlist) {
      Object.keys(_ssrProxying.ALLOWED_CACHING_PROXY_REQUEST_HEADERS).forEach(key => {
        if (!(key in headers)) {
          headers[key] = key;
        }
      });
    }
    const updatedHeaders = (0, _ssrProxying.rewriteProxyRequestHeaders)({
      caching: testCase.caching,
      headers,
      headerFormat: testCase.format || 'http',
  ...
  test('missing headers', () => {
    expect((0, _ssrProxying.rewriteProxyRequestHeaders)({
      appHostname: '',
      proxyPath: '/mobify/proxy/base',
      targetProtocol: 'https',
      targetHost: 'www.customer.com'
