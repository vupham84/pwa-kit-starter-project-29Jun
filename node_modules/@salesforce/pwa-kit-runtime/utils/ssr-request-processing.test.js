...
var _ssrRequestProcessing = require("./ssr-request-processing");
/*
...
describe('Query parameter tests', () => {
  test('Empty querystring', () => {
    const qp = new _ssrRequestProcessing.QueryParameters('');
  ...
  test('Decodes & encodes', () => {
    ...
    const qp = new _ssrRequestProcessing.QueryParameters(qs);
  ...
  test('Special characters handling', () => {
    const test_cases = [{
      ...
      original: "-_.!~*'()",
      // These characters won't be escaped, so they should remain the same.
      escaped: "-_.!~*'()"
    },
    ...
    test_cases.forEach(data => {
      const qp = new _ssrRequestProcessing.QueryParameters(`${data.original}=${data.original}`);
  ...
  test('Filter and from()', () => {
    ...
    const qp1 = new _ssrRequestProcessing.QueryParameters(qs);
    expect(qp1.parameters).toHaveLength(3);
    expect(qp1.keys).toHaveLength(3);
    expect(qp1.toString()).toEqual(qs);
    const qp2 = _ssrRequestProcessing.QueryParameters.from(qp1.parameters.filter(p => p.key !== 'b'));
