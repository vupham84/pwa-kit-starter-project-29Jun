...
const rewriteSetCookies = ({
  ...
  const newCookies = oldCookies.map(cookie => {
    if (cookie.domain) {
      ...
      if (logging) {
        _loggerInstance.default.info(`Rewriting proxy response set-cookie header domain from "${cookie.domain}" to "${newDomain}"`, {
...
const rewriteProxyResponseHeaders = ({
  ...
  if (requestUrl) {
    ...
    logging && _loggerInstance.default.info(`Setting proxy response ${X_PROXY_REQUEST_URL} header to "${fullRequestUrl}"`, {
  ...
  logging && allowOrigin && _loggerInstance.default.info(`Header ${AC_ALLOW_ORIGIN} has value "${allowOrigin}"`, {
    namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
    additionalProperties: {
      allowOrigin,
      AC_ALLOW_ORIGIN
  ...
  if (allowOrigin === targetOrigin) {
    /* istanbul ignore next */
    if (logging) {
      _loggerInstance.default.info(`Rewriting proxy response ${AC_ALLOW_ORIGIN} header to "${appOrigin}"`, {
        namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
        additionalProperties: {
          appOrigin,
          AC_ALLOW_ORIGIN
  ...
  if (statusCode >= 301 && statusCode <= 308) {
    logging && _loggerInstance.default.info(`Status code is ${statusCode}, checking Location header`, {
      namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
      additionalProperties: {
        statusCode
      }
    ...
    logging && _loggerInstance.default.info(`Location header has value "${location}"`, {
      namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
      additionalProperties: {
        location
      }
    ...
    if (location) {
      ...
      if (locUrl.protocol === `${targetProtocol}:` && locUrl.host === targetHost) {
        ...
        /* istanbul ignore else */
        if (logging) {
          _loggerInstance.default.info(`Rewriting proxy response Location header from "${location}" to "${newLocation}"`, {
            namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
            additionalProperties: {
              location,
              newLocation
...
const X_HEADERS_TO_REMOVE_PROXY = exports.X_HEADERS_TO_REMOVE_PROXY = ['x-mobify-access-key', 'x-sfdc-access-control'];
 ...
 * @type {string[]}
 */
const X_HEADERS_TO_REMOVE_ORIGIN = exports.X_HEADERS_TO_REMOVE_ORIGIN = ['x-api-key', 'x-apigateway-event', 'x-apigateway-context', 'x-mobify-access-key', 'x-sfdc-access-control'];
...
const rewriteProxyRequestHeaders = ({
  ...
  if (hostHeader !== targetHost) {
    /* istanbul ignore else */
    if (logging) {
      _loggerInstance.default.info(`Rewriting proxy request Host header from "${hostHeader}" to "${targetHost}"`, {
        namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
        additionalProperties: {
          hostHeader,
          targetHost
  ...
  if (originHeader && originHeader !== targetOrigin) {
    /* istanbul ignore else */
    if (logging) {
      _loggerInstance.default.info(`Rewriting proxy request Origin header from "${originHeader}" to "${targetOrigin}"`, {
        namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
        additionalProperties: {
          originHeader,
          targetOrigin
  ...
  Object.entries(X_HEADERS_TO_ADD).forEach(([key, value]) => {
    /* istanbul ignore else */
    if (logging) {
      _loggerInstance.default.info(`Adding a proxy request ${key} header with value "${value}"`), {
        namespace: 'ssr-proxying.rewriteProxyResponseHeaders',
        additionalProperties: {
          key,
          value
